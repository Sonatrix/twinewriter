...
with col1:
    ...
    has_openai = os.getenv("OPENAI_API_KEY", "").lower() == "true"
    has_anthropic = os.getenv("ANTHROPIC_API_KEY", "").lower() == "true"
    use_ollama = os.getenv("USE_OLLAMA", "").lower() == "true"
    
    if not (has_openai or has_anthropic or use_ollama):
        st.error("⚠️ No LLM configured! Set OPENAI_API_KEY, ANTHROPIC_API_KEY, or USE_OLLAMA=true in .env")
    else:
        if has_openai:
            st.success("✅ LLM: OpenAI (GPT-4)")
        elif has_anthropic:
            st.success("✅ LLM: Anthropic (Claude)")
        elif use_ollama:
    ...
    tone = st.selectbox(
        "Tone",
        options=["professional", "educational", "witty", "marketing", "storytelling", "casual"],
...
with col2:
    ...
    if generate_button and topic:
        ...
        with st.spinner("🤖 Generating content..."):
            try:
                ...
                if not (os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY") or 
                ...
                if final_state and final_state.get("tweets") and not final_state.get("error"):
    ...
    if st.session_state.generated_tweets:        
        ...
        with edit_col1:
            if st.button("✏️ Edit Mode" if not st.session_state.edit_mode else "👁️ View Mode"):
        ...
        for idx, tweet in enumerate(st.session_state.generated_tweets):
            with st.container():
                ...
                with col_a:
                    st.markdown(f'<span class="tweet-number">Tweet {tweet.index}/{len(st.session_state.generated_tweets)}</span>', unsafe_allow_html=True)
                
                with col_b:
                    char_color = "red" if tweet.char_count > 280 else "green"
                    st.markdown(f'<div class="char-count" style="color: {char_color};">{tweet.char_count}/280</div>', unsafe_allow_html=True)
